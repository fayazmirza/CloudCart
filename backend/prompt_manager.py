...
ln -sf "v1.1.0.yaml" "current.yaml"   # from backend/prompts/
"""
...
class PromptManager:
    ...
    def load_prompt(self, version: str = "current") -> Dict[str, Any]:
        """
        Load a versioned prompt YAML from ``prompts_dir`` (flat layout).

        Args:
            version: ``"current"`` loads ``current.yaml`` (symlink to active
                version), or e.g. ``"v1.0.0"`` loads ``v1.0.0.yaml``.

        Returns:
            Dictionary containing prompt data
        """
        if version == "current":
            prompt_file = self.prompts_dir / "current.yaml"
        else:
            prompt_file = self.prompts_dir / f"{version}.yaml"
        ...
        # Add metadata
        prompt_data['loaded_at'] = datetime.now(timezone.utc).isoformat()
        prompt_data['file_path'] = str(prompt_file)
        
        return prompt_data
    
    def get_version_history(self) -> list[str]:
        """List all ``v*.yaml`` version stems under ``prompts_dir``."""
        if not self.prompts_dir.exists():
            return []
        versions = [f.stem for f in self.prompts_dir.glob("v*.yaml")]
